...
def get_week_start(request, prediction_file):
    ...
    if os.path.exists(prediction_file):
        ...
        try:
            ...
            print(
                "Could not determine date from prediction file:",
                e
            )
    ...
    try:
        ...
        print(
            "Could not determine latest database date:",
            e
        )
...
def dashboard(request):
    ...
    if date_param:
        ...
        try:
            ...
            if not pd.isna(param_date):
                ...
                param_date = param_date.normalize()

                week_start = (
                    param_date
                    - pd.Timedelta(
                        days=param_date.weekday()
                    )
                )
    ...
    if not pred_df.empty:
        ...
        pred_grouped["store_id"] = (
            pred_grouped["store_id"]
            .astype(int)
            .astype(str)
        )

        pred_grouped["gate_id"] = (
            pred_grouped["gate_id"]
            .astype(int)
            .astype(str)
        )
    ...
    if (
        ...
        if (
            "actual_csv" in df.columns
            and "actual_db" in df.columns
        ):
    ...
    if not df.empty:
        ...
        df["variation"] = df["variation"].apply(
            lambda x:
                "NA"
                if x == "NA"
...
def get_actual_data(
    ...
    actual_df["gate_id"] = pd.to_numeric(
        actual_df["gate_id"],
        errors="coerce"
    )
